...
from llm import generate_response, generate_response_batch
...
def display_chat_page(pipe):
    ...
    # 質問を複数まとめて入力できるようにする
    num_inputs = st.number_input("質問数", min_value=1, max_value=10, value=1, step=1)
    user_questions = []

    for i in range(num_inputs):
        user_question = st.text_area(f"質問 {i+1}", key=f"question_input_{i}", height=100)
        if user_question:
            user_questions.append(user_question)

    submit_button = st.button("まとめて送信")
    ...
    if submit_button and user_questions:
        ...
        with st.spinner("モデルがまとめて回答を生成中..."):
            answers = generate_response_batch(pipe, user_questions)

            # まとめて保存
            for question, answer in zip(user_questions, answers):
                st.session_state.current_answers.append(answer)
                save_to_db(
                    question,
                    answer,
                    feedback="",  # ここではフィードバックなし
                    correct_answer="",
                    is_correct=None,
                    response_time=0.0  # 詳細は省略（必要なら測定可）
                )
        st.success("回答が生成されました！")

    # 回答が表示される
    if st.session_state.current_questions and st.session_state.current_answers:
        for q, a in zip(st.session_state.current_questions, st.session_state.current_answers):
            st.subheader("質問:")
            st.markdown(q)
            st.subheader("回答:")
            st.markdown(a)
